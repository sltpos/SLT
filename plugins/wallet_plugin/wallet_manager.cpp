 ...
 */
#include <slt/wallet_plugin/wallet_manager.hpp>
...
std::string wallet_manager::create(const std::string& name) {
   ...
   // This can happen if the wallet file is removed while slt-walletd is running.
...
void wallet_manager::open(const std::string& name) {
   ...
   // If we have name in our map then remove it since we want the emplace below to replace.
   // This can happen if the wallet file is added while slt-walletd is running.
   auto it = wallets.find(name);
   if (it != wallets.end()) {
      wallets.erase(it);
   }
