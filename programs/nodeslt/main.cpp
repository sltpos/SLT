/**
 *  @file
 *  @copyright defined in sltpos/LICENSE.txt
 */
#include <appbase/application.hpp>

#include <sltpos/chain_plugin/chain_plugin.hpp>
#include <sltpos/http_plugin/http_plugin.hpp>
#include <sltpos/history_plugin/history_plugin.hpp>
#include <sltpos/net_plugin/net_plugin.hpp>
#include <sltpos/producer_plugin/producer_plugin.hpp>
#include <sltpos/utilities/common.hpp>
...
{
   try {
      app().set_version(sltpos::nodeslt::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "sltpos/nodeslt/data" );
      app().set_default_config_dir(root / "sltpos/nodeslt/config" );
      ...
      initialize_logging();
      ilog("nodeslt version ${ver}", ("ver", app().version_string()));
      ilog("sltpos root is ${root}", ("root", root.string()));
      ilog("nodeslt using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("nodeslt data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
